...
def getLastDBTimeStamp(chanName, pointType, fooStartTime, fooStopTime, fooHistoryFlag):
    ...
    if influxVersion == 2:
        ...
        result = influx.query('select last(usage), time from energy_usage where (device_name = \'' + chanName + '\' AND granularity = \'' + pointType + '\')')
        if len(result) > 0:
    ...
    for chanNum, chan in device.channels.items():
        ...
        if kwhUsage is not None:
            if pointType is None:
                minuteHistoryStartTime, stopTimeMin, minuteHistoryEnabled = getLastDBTimeStamp(chanName, 'minute', stopTime, stopTime, False)
        ...
        if collectDetails and detailedSecondsEnabled and historyStartTime is None:
            ...
            secHistoryStartTime, stopTimeSec, secondHistoryEnabled = getLastDBTimeStamp(chanName, 'second', detailedStartTime, stopTime, detailedSecondsEnabled)
        ...
        # fetches historical Hourly & Daily data
        if historyStartTime is not None and historyEndTime is not None:
            ...
            for kwhUsage in usage:
                if kwhUsage is None:
                    continue
                timestamp = usage_start_time + datetime.timedelta(days=index-1)
...
startupTime = startupTime.replace(microsecond=0)
try:
